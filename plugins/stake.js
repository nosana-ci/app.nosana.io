  ...
  const stake = new Vue({
    ...
    methods: {
      ...
      async setupPrograms () {
        ...
        const poolId = new anchor.web3.PublicKey((Date.now() / 1000 > 1705224600 ? '9k6rgFGZ7posuu9jgnqSmHkcBCE9cw9hWZNeowjhasjW' : process.env.NUXT_ENV_POOL_ID));
      ...
      async refreshRewardsAndPoolInfo () {
        ...
        let rewardAccount;
        try {
          const poolInfo = await this.poolProgram.account.poolAccount.fetch((Date.now() / 1000 > 1705224600 ? '9k6rgFGZ7posuu9jgnqSmHkcBCE9cw9hWZNeowjhasjW' : process.env.NUXT_ENV_POOL_ID));
